...
const SidebarToggle: QuartzComponent = ({ displayClass }: QuartzComponentProps) => {
  return (
    <button class="sidebar-toggle" id="sidebar-toggle" aria-label="Toggle right sidebar">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="0.2" stroke-linecap="round" stroke-linejoin="round">
...
SidebarToggle.afterDOMLoaded = `
  ...
  if (toggle) {
    // Load saved state
    const hidden = localStorage.getItem('hideRightSidebar') === 'true'
    if (hidden) {
    ...
    toggle.addEventListener('click', () => {
      ...
      const isHidden = document.body.classList.contains('hide-right-sidebar')
      localStorage.setItem('hideRightSidebar', isHidden.toString())
    })
  }
`
