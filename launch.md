...
today · 2026-09-21 · 45 days to launch. gates: cyb fleet green (21/21), foculus · tru · mudra · tok · cybergraph green, zheng red in the owner's tree (`cargo test` fails to compile the example `legacy_wire_cost`: `TraceProof: serde::Serialize` not satisfied; `cargo test --lib` passes 198/198), spacepussy-test at height 39,641. red: 30 launch pull requests open, 0 merged since the workers started on 2026-09-18, so no registry row has moved; the foculus PRs (#6–#13) each carry the same Cargo pin fix and will conflict with each other once one merges. red: three privacy audits (zheng #21, foculus #12, mudra #5) say the signal wire format, the settlement ticket and valence all leak by design; P1, P3 and valence depend on zheng private execution, which is itself incomplete. red: the particle definition (row 19) is still an open decision and blocks the re-addressing run (row 20, file #2 ready). the machine rebooted at 16:44 UTC and the worker logs under /tmp were lost; the launchd job is loaded and will tick.
...
| 34 | the public graph is served by every full node and every partial node for its namespaces, replicated by radio, sampled by DAS; a light client verifies against the tip root; the burial file store is the seed, not the only copy | foculus, bbg, radio, cybernode | open | three full nodes on three machines serve the same tip root; a fourth joins from them alone and reaches the same root; DAS sampling passes against a withheld chunk | 2026-10-16 |
| 35 | clusters are canonical: an epoch's claims partition into connected components of overlapping ε-supports, derived from the graph and ε, no miner-drawn boundary; the settlement proof commits to the support | foculus, tru | open | spec §7 exists; code binds a ticket to `claims_root` only; the ε-support derivation and the partition are unimplemented | 2026-10-09 |
| 36 | hardware profile of a ticket measured on the bostrom graph: samples per second and joules per sample on Apple Silicon, x86 desktop, a phone and a GPU; the per-cluster difficulty schedule equalizes pay per second of work | foculus, warriors | open | benchmark in `foculus/audit/`; answers the community's hardware question with numbers | 2026-10-16 |
