...
today · 2026-09-24 · 42 days to launch. gates: cyb fleet green (21/21), foculus · tru · mudra · tok · cybergraph green, zheng red unchanged (owner-tree example `legacy_wire_cost`), spacepussy-test at 40,542 (+870 since yesterday, the fleet gate relaying). the release train exists and ran: soft3 #27–#29 landed `release/phase1.toml`, the qualifier (`release/train*.py`), `specs/releases.md` and the workflows; cyber #111 consumes the soft3 qualification; the first three candidates were cut this morning, `candidate-20260924.1` for cyber, soft3 and cyb, all RED as drafts with receipts, and the red is the truth the train was built to tell: every captured sibling revision is the origin default (bbg 0.2.1, lens 0.1.3, nox 0.2.0, zheng 0.3.3) while the owner's versions sit in unmerged PRs. red: the four version PRs (bbg #9, lens #14, nox #17, zheng #33) are the whole difference between a red and a green candidate. red: the inbox is structured (31 `DECIDE:` PRs assigned to the owner, 30 ready, 184 queue drafts, 9 blocked) but 0 of the reviewed worker PRs have merged in six days. workers paused, plist disabled.
...
| 9 | nodes converge from different starting states | foculus, cybergraph | open | network gate; PR open: [foculus#9](https://github.com/cyberia-to/foculus/pull/9) — convergence order-independent of arrival; audited: bbg's `signals` dimension collided on same-step arrival order across neurons, diverging `root()` between nodes; fixed, keyed by H(neuron‖step) with a regression test (launch #9, bbg/audit/signal-record-key-fix.md); PR open: [bbg#32](https://github.com/cyberia-to/bbg/pull/32) — signals dimension keyed by (neuron, step); cybergraph/inf call-site follow-up still open | 2026-10-02 |
...
|---|---|---|---|
| `decision` | the owner's own branches, version bumps, architecture calls | the owner (review requested) | 45 |
| `ready` | worker PR reviewed by an agent, green, waiting for a merge | the owner clicks, or an agent merges if the owner allows it | 30 |
| `queue` | worker PR not yet reviewed by an agent; kept as a draft | agents review in batches, then move to `ready` or close | 184 |
| `blocked` | draft with a named blocker | the blocker's owner | 7 |
| dependabot | dependency bumps | untouched | 12 |

the `decision` list today: [bbg#22](https://github.com/cyberia-to/bbg/pull/22), [bbg#9](https://github.com/cyberia-to/bbg/pull/9), [cyb#1258](https://github.com/cyberia-to/cyb/pull/1258), [cyb#1280](https://github.com/cyberia-to/cyb/pull/1280), [cyb#1333](https://github.com/cyberia-to/cyb/pull/1333), [cyb#1349](https://github.com/cyberia-to/cyb/pull/1349), [cyb#376](https://github.com/cyberia-to/cyb/pull/376), [cyber-valley#2](https://github.com/cyberia-to/cyber-valley/pull/2), [cybergraph#3](https://github.com/cyberia-to/cybergraph/pull/3), [eidos#1](https://github.com/cyberia-to/eidos/pull/1), [episode-1#7](https://github.com/cyberia-to/episode-1/pull/7), [evy#1](https://github.com/cyberia-to/evy/pull/1), [foculus#3](https://github.com/cyberia-to/foculus/pull/3), [foculus#5](https://github.com/cyberia-to/foculus/pull/5), [fs#1](https://github.com/cyberia-to/fs/pull/1), [glia#1](https://github.com/cyberia-to/glia/pull/1), [go-cyber#796](https://github.com/cyberia-to/go-cyber/pull/796), [go-cyber#800](https://github.com/cyberia-to/go-cyber/pull/800), [hemera#2](https://github.com/cyberia-to/hemera/pull/2), [honeycrisp#11](https://github.com/cyberia-to/honeycrisp/pull/11), [lens#12](https://github.com/cyberia-to/lens/pull/12), [lens#14](https://github.com/cyberia-to/lens/pull/14), [midao#3](https://github.com/cyberia-to/midao/pull/3), [midao#4](https://github.com/cyberia-to/midao/pull/4), [mudra#2](https://github.com/cyberia-to/mudra/pull/2), [neuron#7](https://github.com/cyberia-to/neuron/pull/7), [nox#15](https://github.com/cyberia-to/nox/pull/15), [nox#17](https://github.com/cyberia-to/nox/pull/17), [prysm#3](https://github.com/cyberia-to/prysm/pull/3), [rune#6](https://github.com/cyberia-to/rune/pull/6), [soft3#1](https://github.com/cyberia-to/soft3/pull/1), [soft3#19](https://github.com/cyberia-to/soft3/pull/19), [soft3#3](https://github.com/cyberia-to/soft3/pull/3), [soma#1](https://github.com/cyberia-to/soma/pull/1), [space-pussy#10](https://github.com/cyberia-to/space-pussy/pull/10), [space-pussy#12](https://github.com/cyberia-to/space-pussy/pull/12), [space-pussy#5](https://github.com/cyberia-to/space-pussy/pull/5), [spark#1](https://github.com/cyberia-to/spark/pull/1), [tade#6](https://github.com/cyberia-to/tade/pull/6), [trident#31](https://github.com/cyberia-to/trident/pull/31), [trident#58](https://github.com/cyberia-to/trident/pull/58), [vault#1](https://github.com/cyberia-to/vault/pull/1), [warriors#1](https://github.com/cyberia-to/warriors/pull/1), [zheng#32](https://github.com/cyberia-to/zheng/pull/32), [zheng#33](https://github.com/cyberia-to/zheng/pull/33).

### calendar

| week | lane A · core | lane B · body | lane C · content |
|---|---|---|---|
| 1 · to 09-25 | the train ran (three red candidates 09-24); still slipping on every cell that needs a merge: registry agreed ✓; privacy P1–P3 designed → audits found the gaps instead (rows 10–12, 29), design slipping; property 3 measured ✓ in PR | particle definition frozen (19) slipping: decision open; genesis pipeline one node: not started, replay contract in PR (14) | availability map in PR (cyb#1392); radio into cyb: not started, gossip proven multi-hop (radio#5) |
...
| open | which particle definition wins: hemera(data) 32 bytes, or hemera over the lens commitment — blocks row 20; survey in cybergraph#4. found 2026-09-24: every crate computes hemera(data) today (file, bbg, cybergraph); only cyber/particle.md states the lens construction. the choice also fixes the layer of lens: hemera(data) keeps lens with the warrior (nox look provider); hemera over the commitment makes lens substrate below the language |
...
| 2026-09-24 | first candidates cut by the train: cyber, soft3, cyb `candidate-20260924.1`, all red drafts; the qualifier binds each candidate to the consumed stack revision and names the drifted siblings; a green candidate needs the four version PRs merged first |
| 2026-09-24 | the workers had been resumed by another session and opened ~150 more PRs overnight; stopped again, launchd plist disabled, prompt throttled (3 queue PRs per repo, no second PR per row). the pull request inbox is structured: four labels across the org, review requested from the owner only on `decision`; dashboard in the queue section above |
...
| 2026-09-24T05:31:03Z | 21 | soft3 | [launch #21 (5): soft3's first radio dependency — gossip topic id](https://github.com/cyberia-to/soft3/pull/26) | open |
