...
2. Start with your lane preference. Within a lane prefer the earliest close-by, then the row whose next step is smallest and clearest. When your lane has nothing tractable, take the next lane; do not return NONE while any lane has a tractable open row. Spec, audit and measurement slices count: a row whose code step is blocked usually still has a spec clause, a test, a measurement or a migration script to write.
...
## never
Publish crates, yank, deploy, touch servers, change DNS, rewrite history, open more than three PRs, work on a taken or claimed row, edit an owner's working tree, add dependencies from outside the org without saying so in the PR, or spend past 45 minutes: at 45 minutes push what you have as a draft PR titled `launch #<n> (draft): …` with a clear Remains section.
