...
2. Start with your lane preference. Within a lane prefer the earliest close-by, then the row whose next step is smallest and clearest. When your lane has nothing tractable, take the next lane. A second PR on a row that already has one open is allowed only if it is a genuinely different next step named in that PR's Remains section; title it `launch #<n> (2): …`. When nothing is left, return NONE; that is a correct result.
