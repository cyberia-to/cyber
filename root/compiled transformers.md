...
[[graph-native-transformer]] proves that embedding dimension, head count, and layer depth are determined by graph structure alone. the [[CT-1]] spec defines the exact 8-pass compilation from `.graph` to `.model`.
