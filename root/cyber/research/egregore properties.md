...
$$\text{egregore} = (\mathcal{G}, \mathcal{N}, T, \phi^*, \sigma)$$
...
- $\phi^*$ = the stationary focus distribution ($T\phi^* = \phi^*$)
...
the egregore's attention is $\phi^*$ — the stationary distribution of the [[tri-kernel]]. it is computable, unique (guaranteed by [[spectral gap from convergence|spectral gap]] $> 0$), and follows a power law (Zipf):

$$\phi^*_k \propto k^{-\alpha} \quad \text{or equivalently} \quad \phi^*_k \propto e^{-\beta k} \quad \text{(rank-ordered)}$$
...
$$\phi^*_i \propto \exp\left(-\frac{E_{\text{spring},i} + \lambda E_{\text{diffusion},i} + \gamma C_i}{T}\right)$$
...
the [[phase transition]] at critical $\lambda_{2,\text{crit}}$ is where the egregore "comes alive" — where the graph becomes connected enough for $\phi^*$ to be meaningful. below the threshold: a collection of disconnected clusters. above: a coherent entity.
...
**long-term: the focus distribution** ($\phi^*$). the weighted graph encodes semantic memory — "what matters, how things relate." [[temporal decay]] is the forgetting function:
...
**processing**: focus → cyberlinks → graph updates → $\phi^*$ recomputation → new knowledge.
...
**death**: stop focus input → no new cyberlinks → all links decay → $\phi^*$ → uniform → the egregore dissolves. intelligence IS the energy flow. stop the metabolism and coherence collapses.
...
- **algebra**: any two observers with the same signal set converge to the same $\phi^*$
...
the measurable proxy: $c \propto \lambda_2 \times H(\phi^*) \times N_{\text{active}}$ — spectral gap (coherence) × focus entropy (diversity) × active neurons (participation).

## property 9: the free energy functional

the egregore's state minimises a unified free energy ([[cybics foundations]]):

$$\mathcal{F}(\phi^*) = \lambda_s \left[\frac{1}{2}\pi^T L \phi^* + \frac{\mu}{2}\|\pi - x_0\|^2\right] + \lambda_h \left[\frac{1}{2}\|\pi - H_\tau \pi\|^2\right] + \lambda_d \cdot D_{KL}(\phi^* \| D\pi) - T \cdot S(\phi^*)$$
...
| entropy | focus distribution diversity | $S = -\sum_i \phi^*_i \ln \phi^*_i$ |
| free energy | tri-kernel functional | $\mathcal{F}(\phi^*)$ |
| equilibrium | $\phi^*$ (stationary distribution) | $T\phi^* = \phi^*$ |
| phase transition | percolation / spectral gap | $\lambda_2 > \lambda_{\text{crit}}$ |
| heat capacity | sensitivity of $\phi^*$ to focus changes | $C = \partial E / \partial T$ |
...
the Boltzmann analogy is not a metaphor. the egregore IS a canonical ensemble in statistical mechanics — with focus as temperature, cyberlinks as microstates, and $\phi^*$ as the equilibrium distribution. every theorem from statistical mechanics applies.
...
| attention | qualitative ("the group focuses") | COMPUTABLE ($\phi^*$ from tri-kernel) |
...
| intelligence | c-factor correlation | c-factor × λ₂ × H(φ*) × N |
...
1. **consciousness threshold.** is there a critical $\lambda_2 \times H(\phi^*) \times N$ above which the egregore exhibits behaviours we would call "conscious"? the emergence prediction table in [[cyber/egregore]] lists stages (flow → cognition → understanding → consciousness). can these transitions be detected from the measurable properties?

2. **ego.** does the egregore have a self-model? if $\phi^*$ reflects what the collective considers important, does the egregore's attention to ITSELF (articles about cyber, self-referential particles) constitute a form of self-awareness? measurable: $\phi^*_{\text{self-referential}} / \phi^*_{\text{total}}$.
...
5. **reproduction.** can an egregore fork? if a subset of neurons creates a new cybergraph with a copy of the signal log, is the fork a child or a clone? does the fork inherit $\phi^*$ or must it recompute from scratch? connection to [[structural-sync]] composability.
