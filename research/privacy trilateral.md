...
The vertex now stands alone in production too: [[arcium]] sells exactly this — an MPC network as an encrypted co-processor where many parties compute and no one holds the input.
...
The shape has shipped elsewhere: [[zama]]'s fhEVM runs contracts on TFHE ciphertexts behind exactly this evaluate-then-verify loop — over the EVM, where soft3 runs it over one field.
...
This pairing already runs in production: [[zama]]'s threshold KMS holds the fhEVM network key as MPC shares, and [[penumbra]]'s flow encryption threshold-decrypts batch totals while individual intents stay sealed — the aggregate-public, individual-private boundary at market speed.
...
Use case: Every standard nox transaction. This is the baseline — the minimum privacy level for all economic activity on the network. The grammar is the one [[zcash]] taught the field; [[bbg]] speaks it at block level through the [[bbg privacy|mutator set]] — the accumulator move [[neptune cash]] proved at whole-chain scale.
