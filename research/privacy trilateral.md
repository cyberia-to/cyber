...
A decade of chains paid for this map one vertex at a time — [[privacy]] keeps their table of lessons. This page derives why the vertices need each other.
...
The vertex now stands alone in production too: [[privacy/arcium]] sells exactly this — an MPC network as an encrypted co-processor where many parties compute and no one holds the input.
...
The shape has shipped elsewhere: [[privacy/zama]]'s fhEVM runs contracts on TFHE ciphertexts behind exactly this evaluate-then-verify loop — over the EVM, where soft3 runs it over one field.
...
This pairing already runs in production: [[privacy/zama]]'s threshold KMS holds the fhEVM network key as MPC shares, and [[privacy/penumbra]]'s flow encryption threshold-decrypts batch totals while individual intents stay sealed — the aggregate-public, individual-private boundary at market speed.
...
Use case: Every standard nox transaction. This is the baseline — the minimum privacy level for all economic activity on the network. The grammar is the one [[privacy/zcash]] taught the field; [[bbg]] speaks it at block level through the [[bbg privacy|mutator set]] — the accumulator move [[privacy/neptune]] proved at whole-chain scale.
...
This is why the [[Goldilocks field processor|GFP]] accelerates the entire privacy stack with four hardware primitives:
