...
We built a public ledger for coins and left the ledger of meaning in private hands. A transfer of value settles without a bank; a transfer of understanding still rents its rank — what deserves attention, who taught the pattern, who is paid when the shared picture sharpens — from whoever owns the feed and the weight file trained on everyone for free. We propose cyber: [[neurons]] grow one [[cybergraph]] of content-addressed [[particles]] joined by signed [[cyberlinks]]. [[Tru]] computes a unique collective [[focus]] $\phi^*$ via the [[tri-kernel]]; that [[fixed point]] ranks the graph and [[tru/docs/explanation/graph-native-transformer|compiles a model]] from structure. Two ways to earn [[cyber/$CYB|$CYB]], and they are not the same job. [[Staking]] is a bet on truth: you lock capital on a link, gain influence if the network’s focus moves with you, and lose if it does not — risk required. [[Mining]] is capital-free and risk-free useful work: you compute the fair [[Shapley value|division]] of proven focus shift among who taught it, then [[fold mining|fold]] those proofs into a mint claim — no bag needed, only correct arithmetic. Emission pays the calculator who settles credit; skin in the game pays the teacher who risked being wrong. [[Foculus]] makes $\phi^*$ the rule of finality: a fact stands when enough attention has gathered — agreement is local computation and gossip, not a vote that must cross the solar system and return. That is how one mind can span light-minutes. It only stays one mind if no one owns the gate: [[soft3]] wraps the stack in [[privacy trilateral|privacy]] so a neuron may teach without fear, and a staked link enters by math — no committee to refuse it, no platform to unpublish it; the only answer to a claim is a heavier claim. That is the bar for [[interplanetary superintelligence]]: shared focus across the dark between worlds, and speech no empire can mute.
