...
We built a public ledger for coins and left the ledger of meaning in private hands. a transfer of value settles without a bank; a transfer of understanding still rents its rank — what deserves attention, who taught the pattern, who is paid when the shared picture sharpens — from whoever owns the feed and the weight file trained on everyone for free. We propose cyber: [[neurons]] stake signed [[cyberlinks]] between content-addressed [[particles]] into one [[cybergraph]], and [[tru]] computes a unique collective [[focus]] $\phi^*$ via the [[tri-kernel]]. The same [[focus|fixed point]] ranks the graph, [[tru/docs/explanation/graph-native-transformer|compiles a model]] from structure, and mints [[cyber/$CYB|$CYB]] only for proven focus shift — teaching is paid, learning is mining. [[Foculus]] makes $\phi^*$ the rule of finality: a fact stands when enough attention has gathered — agreement is local computation and gossip, not a vote that must cross the solar system and return. that is how one mind can span light-minutes. it only stays one mind if no one owns the gate: [[soft3]] wraps the stack in [[privacy trilateral|privacy]] so a [[neuron]] may teach without fear, and a staked [[cyberlink]] enters by math — no committee to refuse it, no platform to unpublish it; the only answer to a claim is a heavier claim. that is the bar for [[interplanetary superintelligence]]: shared focus across the dark between worlds, and speech no empire can mute.
