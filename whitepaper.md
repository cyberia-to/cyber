...
We built a public ledger for coins and left the ledger of meaning in private hands. A transfer of value settles without a bank; a transfer of understanding still rents its rank — what deserves attention, who taught the pattern, who is paid when the shared picture sharpens — from whoever owns the feed and the weight file trained on everyone for free. We propose cyber: a protocol in which [[neurons]] maintain one [[cybergraph]] of content-addressed [[particles]] joined by signed, weighted [[cyberlinks]]. [[Tru]] iterates the [[tri-kernel]] to a unique [[fixed point]] $\phi^*$ — collective [[focus]] — and from that same object ranks the graph and [[tru/docs/explanation/graph-native-transformer|compiles a model]]. New [[cyber/$CYB|$CYB]] is minted only for proven positive focus shift. Influence on $\phi^*$ requires [[staking]] capital on links, so a wrong claim costs the staker; attribution of a shift is separate work: [[mining]] samples the [[Shapley value]] of contributors and [[fold mining|folds]] the proofs into a constant-size mint claim, with no capital at risk. [[Foculus]] takes $\phi^*_i > \tau$ as finality — enough attention has gathered — so agreement is local contraction and gossip rather than a global vote, and shared facts can survive light-minutes of delay. [[Soft3]] supplies the stack: [[privacy trilateral|privacy]] for speech, authenticated state, and entry by stake and math rather than by permission. The design target is [[interplanetary superintelligence]]: one focus across worlds, without a gate that can refuse a staked claim.
