...
// For relative paths starting with ./, don't allow going above the starting point
function normalizePath(path: string): string {
  const startsWithDotSlash = path.startsWith("./")
  const startsWithDotDot = path.startsWith("../")
  ...
  for (const segment of segments) {
    if (segment === "..") {
      ...
      if (result.length > 0 && result[result.length - 1] !== "..") {
        result.pop()
      } else if (startsWithDotDot) {
        // Keep ".." only for paths that started with ../
        result.push("..")
      }
      // For paths starting with ./, don't add .. (we're already at root)
    } else {
      result.push(segment)
    }
  }
  ...
  // Preserve relative prefix
  const normalized = result.join("/")
  if ((startsWithDotSlash || startsWithDotDot) && !normalized.startsWith(".")) {
    return "./" + normalized
  }
  return normalized
}
...
const _rebaseHastElement = (
  ...
  if (el.properties?.[attr]) {
    ...
    el.properties[attr] = normalizePath(rel)
  }
}
...
export function transformLink(src: FullSlug, target: string, opts: TransformOptions): RelativeURL {
  ...
  if (opts.strategy === "relative") {
    ...
    // Check if source is a folder page (has child pages under it)
    // If so, use src/index for path calculation since it will output to src/index.html
    let effectiveSrc = src
    const srcIsFolder = opts.allSlugs.some((slug) =>
      slug !== src && slug.startsWith(src + "/")
    )
    if (srcIsFolder && !src.endsWith("/index")) {
      effectiveSrc = (src + "/index") as FullSlug
    }
    ...
    if (opts.strategy === "shortest") {
      ...
      if (matchingFileNames.length === 1) {
        ...
        return (resolveRelative(effectiveSrc, targetSlug) + targetAnchor) as RelativeURL
      }
    }
