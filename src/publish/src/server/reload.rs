...
fn watch_and_rebuild_loop(config: &SiteConfig, build_version: &Arc<AtomicU64>) -> Result<()> {
    ...
    let pages_dir = config.build.input_dir.join("pages");
    let journals_dir = config.build.input_dir.join("journals");

    if pages_dir.exists() {
        watcher.watch(&pages_dir, notify::RecursiveMode::Recursive)?;
    }
    if journals_dir.exists() {
        watcher.watch(&journals_dir, notify::RecursiveMode::Recursive)?;
    }
