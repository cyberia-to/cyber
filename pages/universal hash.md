...
- # UniversalHash v4: A Democratic Proof-of-Work Algorithm
  ...
  ---
- ## Table of Contents
- [Introduction](https://claude.ai/chat/9e947f67-863a-4bcb-b480-0f32d4ebcb0b#1-introduction)
- [Problem Statement](https://claude.ai/chat/9e947f67-863a-4bcb-b480-0f32d4ebcb0b#2-problem-statement)
- [Design Goals](https://claude.ai/chat/9e947f67-863a-4bcb-b480-0f32d4ebcb0b#3-design-goals)
- [Comparative Analysis](https://claude.ai/chat/9e947f67-863a-4bcb-b480-0f32d4ebcb0b#4-comparative-analysis)
- [Algorithm Specification](https://claude.ai/chat/9e947f67-863a-4bcb-b480-0f32d4ebcb0b#5-algorithm-specification)
- [Security Analysis](https://claude.ai/chat/9e947f67-863a-4bcb-b480-0f32d4ebcb0b#6-security-analysis)
- [Performance Projections](https://claude.ai/chat/9e947f67-863a-4bcb-b480-0f32d4ebcb0b#7-performance-projections)
- [Economic Model: Self-Regulating Proof Density](https://claude.ai/chat/9e947f67-863a-4bcb-b480-0f32d4ebcb0b#8-economic-model-self-regulating-proof-density)
- [Bostrom Integration: Pool-less Epoch Mining](https://claude.ai/chat/9e947f67-863a-4bcb-b480-0f32d4ebcb0b#9-bostrom-integration-pool-less-epoch-mining)
- [Interplanetary Consensus: Adaptive Epochs](https://claude.ai/chat/9e947f67-863a-4bcb-b480-0f32d4ebcb0b#10-interplanetary-consensus-adaptive-epochs)
- [Implementation: Verifier (Rust)](https://claude.ai/chat/9e947f67-863a-4bcb-b480-0f32d4ebcb0b#11-implementation-verifier-rust)
- [Implementation: Prover (Multi-Platform)](https://claude.ai/chat/9e947f67-863a-4bcb-b480-0f32d4ebcb0b#12-implementation-prover-multi-platform)
- [Integration Guidelines](https://claude.ai/chat/9e947f67-863a-4bcb-b480-0f32d4ebcb0b#13-integration-guidelines)
- [Future Work](https://claude.ai/chat/9e947f67-863a-4bcb-b480-0f32d4ebcb0b#14-future-work)
- [Conclusion](https://claude.ai/chat/9e947f67-863a-4bcb-b480-0f32d4ebcb0b#15-conclusion)
...
- ### 4.3 UniversalHash v4 Design Decisions
  ...
  | 
  | 2MB total memory (4×512KB) | 
  | Fits all phone L3 caches; larger than XelisHash for margin | 
  |
  
  | 
  | 4 parallel chains | 
  | Matches budget phone core count; caps server advantage | 
  |
  
  | 
  | AES + SHA-256 + BLAKE3 | 
  | All have hardware acceleration; BLAKE3 especially ASIC-hostile | 
  |
  
  | 
  | Sequential inner loop | 
  | Prevents GPU parallelism within single hash | 
  |
  
  | 
  | Simple XOR merge | 
  | Auditable finalization; no complex reduction function | 
  |
...
- ### 8.7 Economic Attack Resistance
  ...
  | 
  | Spam minimum-difficulty proofs | 
  | Flood mempool with cheap proofs | 
  | Gas price rises → min_profitable_difficulty rises → spam unprofitable | 
  |
  
  | 
  | Self-dealing (submit to self as validator) | 
  | Validator includes own low-diff proofs | 
  | Still pays gas to network; other validators can reject | 
  |
  
  | 
  | Difficulty gaming | 
  | Withhold proofs to manipulate next epoch | 
  | Proofs timestamped; late submission rejected | 
  |
  
  | 
  | Sybil proofs | 
  | Split hashrate across addresses | 
  | No advantage; same total difficulty, same rewards | 
  |
- ### 8.8 Summary: Self-Regulating Economics
  ...
  | Proof = authentication; gas deducted from rewards | 
  |
  
  | 
  | Spam resistance | 
  | Economic threshold rejects unprofitable proofs | 
  |
  
  | 
  | Load balancing | 
  | EIP-1559 adjusts gas → adjusts proof density | 
  |
  
  | 
  | Fair distribution | 
  | Difficulty-weighted rewards; no pool needed | 
  |
...
- Variance reduction (small miners find blocks infrequently)
- Payout smoothing
- Infrastructure (stratum servers, etc.)
  ...
  | Proof submission | 
  | Must win race | 
  | Submit anytime in epoch | 
  |
  
  | 
  | Variance | 
  | Winner-take-all per block | 
  | Proportional per epoch | 
  |
  
  | 
  | Infrastructure | 
  | Need pool servers | 
  | Use existing PoS chain | 
  |
...
- ### 9.4 Epoch Parameters
  ...
  | 
  | Epoch duration | 
  | 10 min (120 PoS blocks) | 
  | Fast feedback loop, good UX | 
  |
...
- Parallel verification: Multiple proofs can reference same parents
  ...
  | Max parents | 
  | 3 | 
  | Limit verification cost | 
  |
  
  | 
  | Parent selection | 
  | Random from recent 100 | 
...
- ### 9.10 Summary: Bostrom PoW Design
  ...
  | 
  | Consensus | 
  | Bostrom PoS (unchanged) | 
  |
  
  | 
  | PoW role | 
  | Fair distribution + device verification | 
  |
  
  | 
  | Block production | 
  | None (proofs are transactions) | 
  |
  
  | 
  | Pooling | 
  | Not needed (proportional rewards) | 
  |
  ...
  | 
  | Target proof rate | 
  | ~500 tx/s (~300K proofs/epoch) | 
  |
  
  | 
  | Gas model | 
  | Deferred (deducted from rewards) | 
  |
  
  | 
  | DAG structure | 
  | Optional, for ordering within epoch | 
  |
  
  | 
  | Difficulty adjustment | 
  | Per-epoch, targeting proof count | 
  |
  
  | 
  | Rewards | 
  | Proportional to difficulty-weighted work minus gas | 
  |
...
- ### 10.7 Phased Deployment
  ...
  | 
  | Phase 2 (Mars prep) | 
  | 10 min + deterministic seeds | 
  | Enable zero-wait mining | 
  |
  
  | 
  | Phase 3 (Mars live) | 
  | Adaptive (10-60 min) | 
  | Astronomical computation on-chain | 
  |
  
  | 
  | Phase 4 (Outer solar system) | 
  | Extended (1-6 hours) | 
  | Jupiter, Saturn colony support | 
  |
- ### 10.8 Summary: Interplanetary Design
  ...
  | 
  | Base epoch (Earth-only) | 
  | 10 minutes | 
  | Fast UX, low variance | 
  |
  
  | 
  | Adaptive epoch | 
  | 10-60 minutes | 
  | Based on Earth-Mars distance | 
  |
  
  | 
  | Epoch seed | 
  | Deterministic | 
  | Zero-wait mining for all locations | 
  |
  
  | 
  | Grace period | 
  | +max_latency | 
  | Accept late proofs with valid timestamps | 
  |
  
  | 
  | Epoch computation | 
  | On-chain astronomical model | 
  | Deterministic, no coordination | 
  |
...
- Block hash is valid (full computation)
- OR trust scratchpad commitment from full nodes (SPV-style)
...
- Production-ready with multi-platform support
  ...
  ---
- ## References
- Tevador et al., "RandomX: A Proof-of-Work Algorithm Designed for General-Purpose CPUs," 2019
- Toutonghi, M., "VerusHash: Phase I zk-SNARK Privacy," Verus Foundation, 2019
- XELIS Project, "XelisHash v3 Specification," 2024
- Dryja, T., "Efficient Authenticated Dictionaries with Skip Lists and Commutative Hashing," MIT DCI, 2017
- Biryukov, A., Khovratovich, D., "Argon2: the memory-hard function for password hashing," 2015
