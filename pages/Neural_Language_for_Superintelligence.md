...
This precision comes at a cost. Goedel's incompleteness theorems prove that no sufficiently powerful formal system can be both complete and consistent. Any formal language capable of expressing arithmetic contains true statements it cannot prove. This is not a bug to be fixed but a fundamental limit on what formal systems can express.
...
fn query_fermentation(graph: &Cybergraph) -> Vec<Particle> {
    ...
    graph.follow_motif(fermentation, causes)
    ...
    for (const inn of inLinks) {
      ...
      if (bridgeLinks.length > 0) {
        motifs.push({
          ...
          particles: [inn.from, particleCid, out.to],
...
fn transitive_causation(a: Particle, c: Particle) -> Option<LinkChain> {
    ...
    a.chains_to(c)
     .filter(|chain| chain.uses_semcon(causes))
     .shortest()
}

// Self-referential meta-knowledge
fn dispute(claim: CyberLink, reason: Particle) -> Sentence {
    let claim_particle = claim.as_particle();  // link becomes particle
    let contradicts = resolve("contradicts");

    sentence [
        claim_particle -> contradicts,
        contradicts -> reason,
        reason -> TRUE,
    ]
}
...
impl FocusComputer {
    ...
    fn converge(&self, graph: &CyberGraph, tolerance: f64)
        -> FocusVector
    {
        let mut phi = FocusVector::uniform(graph.particle_count());
