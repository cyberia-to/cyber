...
> *Inspired by the mathematical foundations of patch theory (Pierre-Étienne Meunier et al.),  
> built from first principles for the CORE ecosystem.*

---

## Table of Contents

1. [Motivation and First Principles](#1-motivation-and-first-principles)
2. [Mathematical Foundations](#2-mathematical-foundations)
3. [Core Ontology](#3-core-ontology)
4. [Identity and Cryptography](#4-identity-and-cryptography)
5. [Patch Algebra](#5-patch-algebra)
6. [Graph Model](#6-graph-model)
7. [Channel Theory](#7-channel-theory)
8. [Content Addressing and Transport](#8-content-addressing-and-transport)
9. [Consensus Integration](#9-consensus-integration)
10. [Agent Interface](#10-agent-interface)
11. [Cyber License Compatibility](#11-cyber-license-compatibility)
12. [Appendix: Formal Definitions](#12-appendix-formal-definitions)
...
The CORE cybergraph already models knowledge as:

- **Particles**: content-addressed knowledge atoms
...
- **Objects** are repository states `S` (sets of tracked content atoms)
...
```
/// A content-addressed atom of tracked data
Atom {
...
Operation {
    kind: OperationKind,
    target: CID,           // CID of atom being affected
    payload: Option<CID>,  // CID of new content (for additions/replacements)
}

OperationKind = 
    | AddAtom       // introduce new atom to tracked set
    | RemoveAtom    // remove atom from tracked set  
    | AddEdge(from: CID, to: CID, kind: EdgeKind)   // link two atoms
    | RemoveEdge(from: CID, to: CID, kind: EdgeKind)
    | ReplaceContent(old: CID, new: CID)             // atomic content swap
...
/// A repository — collection of channels over a shared atom space
Repository {
    id:           CID,              // hash of genesis state
    channels:     Map<ChannelName, Channel>,
    atoms:        Set<CID>,         // union of all tracked atoms
...
Atom        ↔  Particle (content-addressed node)
...
This embedding is not metaphorical — CyberPatch repositories ARE cybergraph structures and can be queried, ranked, and rewarded by the CORE consensus layer directly.
...
- On-chain identity: derived from signing key, registered via CORE consensus
...
Neuron IDs are resolved to public keys through the CORE name system, supporting:
...
Op = AddAtom(cid: CID)
   ...
   | ReplaceAtom(old: CID, new: CID)
```

**Invariants:**
- `RemoveAtom(x)` requires `AddAtom(x)` in dependency closure
- `RemoveEdge(a,b,l)` requires `AddEdge(a,b,l)` in dependency closure
- `ReplaceAtom(old, new)` requires `AddAtom(old)` in dependency closure
- Edges can only reference atoms present in the current state
...
| op₁ \ op₂      | AddAtom(y) | RemoveAtom(y) | AddEdge(a,b,l) | RemoveEdge(a,b,l) | ReplaceAtom(old,new) |
|----------------|-----------|--------------|---------------|------------------|-------------------|
| AddAtom(x)     | x≠y: ✓   | x≠y: ✓      | x∉{a,b}: ✓   | x∉{a,b}: ✓      | x∉{old,new}: ✓   |
| RemoveAtom(x)  | x≠y: ✓   | x≠y: ✓      | x∉{a,b}: ✓   | x∉{a,b}: ✓      | x≠old: ✓          |
...
```
ConflictKind =
    | DoubleAdd(cid: CID)          // two patches add same atom with different content
    | DeleteDelete(cid: CID)       // two patches delete same atom (benign — same result)
    | EditDelete(cid: CID)         // one patch edits, another deletes
    | DoubleEdit(cid: CID)         // two patches replace same atom differently
...
- `V ⊆ CID`: set of atom content identifiers (vertices)
...
create_node(content: Bytes) → AddAtom(BLAKE3(content))
...
```
FilesystemView {
    // Maps filesystem paths to atom CIDs
    tree: Map<Path, CID>
}
```

A filepath change is a patch to the tree map, not to content. Content changes are patches to atom content. The two are independent and can be composed:

```
// Rename without changing content = patch to tree only
rename("/old/path", "/new/path") → ReplaceAtom(path_atom_old, path_atom_new)

// Change content without renaming = patch to atom content only  
edit("/path", new_content) → ReplaceAtom(content_cid_old, new_content_cid)
```
...
The CORE tri-kernel ranking assigns a focus weight to each patch based on its impact on the knowledge graph:
...
Where:
- `diffusion_score`: measures how widely this patch's atoms are referenced (exploration)
...
Voting weight = neuron's stake × focus_weight. This ties version control conflict resolution directly to CORE's economic and epistemic consensus layer.
...
Patches that increase network knowledge coherence (positive Δπ) earn rewards. Patches that fragment or duplicate existing knowledge earn less or nothing. This creates an economic pressure toward high-quality, well-connected knowledge contributions — directly aligned with CFT predictions.
...
session.add_atom(content: Bytes) → CID
session.remove_atom(cid: CID)
...
```
Agent₁: ops on atoms {a, b, c}  →  Patch P₁(deps: ∅)
Agent₂: ops on atoms {d, e, f}  →  Patch P₂(deps: ∅)
Agent₃: ops on atoms {a, g}     →  Patch P₃(deps: ∅)

// P₁ and P₂ are independent: they can be applied in any order
// P₁ and P₃ may conflict (both touch atom 'a')
...
This directly implements the CORE design directive of GFlowNet-weighted patch proposals.
...
- P-É. Meunier — mathematical theory of patches, categorical VCS foundations
...
Key properties of Cyber License (as intended by the CORE project):
...
| Atom | Content-addressed unit of tracked data (vertex) |
...
*CORE Ecosystem — Collective Objective Reality Engine*  
